...
export class VentaController {
    ...
    static newVenta = async (req: Request, res: Response) => {
        const { idProducto, cantidad, userId } = req.body;
        ...
        try {
            venta = await ventaRepo.createQueryBuilder('venta')
                .innerJoinAndSelect('venta.user', 'user')
                .where('venta.user=:idv', { idv: userId })
                .groupBy('venta.id')
                .getMany();
            existencia = venta.length + 1;
        } catch (e) {
            console.log('e: ' + e);
        ...
        try {
            for (let i = 0; i < idProducto.length; i++) {
                cant[i] = await productoRepo.findOneOrFail(idProducto[i], { select: ['cantidad', 'costo', 'minExistencia', 'descripcion'] });
            }
        } catch (e) {
            console.log('error productos: ' + e);
            return res.status(404).json({ message: 'no se encontro el/los producto/s' });
        }

        //console.log('cant: ' + JSON.stringify(cant));

        if (cant.length > 0) {
            try {
                try {
                    for (let j = 0; j < idProducto.length; j++) {
                        for (let i = 0; i < cantidad[j]; i++) {
                            venta = await ventaRepo
                                ...
                                .values([
                                    {
                                        fechaVenta: fecha,
                                        //productoId: parseInt(idProducto[j]),
                                        id: existencia,
                                        //precio: cant[j].costo,
                                        //userId: userId
                                    }
                    ...
                    try {
                        for (let i = 0; i < idProducto.length; i++) {
                            prod = await productoRepo
                                .createQueryBuilder()
                                .update(Producto)
                                .set({ cantidad: cant[i].cantidad - cantidad[i], modificado: fecha })
                                .where('id=:id', { id: parseInt(idProducto[i]) })
                                .execute();
                        }
                    } catch (e) {
                        console.log('error de actualizacion: ' + e);
                        return res.status(404).json({ message: 'error en la venta' });
                    }

                } catch (e) {
                    console.log('error en transacción: ' + e);
                }

            }
            catch (e) {
                console.log('e2: ' + e);
                return res.status(409).json({ message: 'no se pudo guardar venta' })
            }
            res.status(200).json({ message: 'Se cargo la venta' });
        } else {
